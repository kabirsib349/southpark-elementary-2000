...
function renderTeachers() {
  ...
  grid.innerHTML = teachers.map(t => `
    <div class="teacher-card">
      ...
      <div class="star-rating">${renderStars(t.rating)}</div>
      <div style="font-size:11px; color:#88ff88; margin:5px 0;">${t.subject} • ${t.yearsAtSchool} ans a l'ecole</div>
      <div class="teacher-quote">"${t.quote}"</div>
      <div style="font-size:10px; color:#66ff66; margin-top:6px; font-style:italic;">*** ${t.funFact}</div>
    </div>
  `).join('');
}
...
async function initVisitorCounter() {
  ...
  try {
    const response = await fetch('https://api.countapi.xyz/hit/southpark-elementary/visits');
    const data = await response.json();
    visitorCount = data.value || 1337;
  } catch (e) {
